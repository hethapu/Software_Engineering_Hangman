...
class Game {
	...
	def Guess(String newguess)
	{
		...
		if(!invalidGuess)
			{
				if(!Phrase.phrasevalue.toUpperCase().contains(newguess.toUpperCase()))
				{
					BadGuesses++
					if(BadGuesses >= 10)
					{
						TimeSpent = TimeCategory.minus(new Date(), Start)
						Complete = true
						Won = false
					}
				...
				{
					if(Phrase.phrasevalue == CurrentView())
					{
						TimeSpent = TimeCategory.minus(new Date(), Start)
						Complete = true
						Won = true
	...
	def String CurrentView()
	{
		...
		if(Regex == '[]')
		  return (Phrase.phrasevalue)
		else
		  return (Phrase.phrasevalue.replaceAll(~Regex, '_'))
	}
	
	def String SpacedView()
	{
		String CV = CurrentView()
		CV.replaceAll("(.)", '$1&nbsp;')
	}
}
